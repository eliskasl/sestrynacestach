...
<p>This option is supported in both shortcode (a) and shortcode (b)</p>
 ...
 ?>
    ...
    function cws_gpp_upgrade_content() {
        ...
        $strOutput = "<h3><span>Get Google Picasa Pro!</span></h3>
                    <div class=\"inside\">
                        ...
                        <a href=\"http://www.cheshirewebsolutions.com/?utm_source=wp_gp_viewer&utm_medium=wp_plugin&utm_content=meta_box_download_it_here&utm_campaign=plugin_upgrade\">Download it here</a> <span><strong>GET 25% OFF!</strong> – use discount code <strong>WPMEGA25</strong> on checkout</span></p>
                        <h3>
                            Reasons to UPGRADE!
                        </h3>
                        <ol>
    ...
    public function getAlbumImages( $AccessToken, $album_thumb_size, $show_album_ttl, $cws_page, $num_image_results, $cws_album, $imgmax='450', $theme ) {

        // Crop the image or not, projig wants not cropped...
        if ( $theme != 'projig' ) { $crop = 'c'; } else { $crop = ''; }

        if( $imgmax == 0 || $imgmax == 0 ) { $imgmax = 450; }
...
// echo $post->post_name;
        ...
        if( $previous > 0 ) {
            ...
            if( $album_id ) {
                $html[] = "<li><a href=\"?cws_page=$previous&cws_album=$album_id\" id='prev_page'>Previous</a></li>";
            } else {
        ...
        for( $i=1; $i <= $num_pages; $i++ ) {
            ...
            if( $album_id ) {
                $html[] = "<a href=\"?cws_page=$i&cws_album=$album_id\" id='page_".$i."'>".$i."</a></li>\n";
            } else {
        ...
        if( $next <= $num_pages ) {
            ...
            // if have album id
            if( $album_id ){
                $html[] = "<li><a href=\"?cws_page=$next&cws_album=$album_id\" id='next_page'>Next</a></li>";
            } else {
                ...
                if ( ! get_user_meta( $userObj->ID, 'cws_gpp_ignore_upgrade' ) ) {
                    ...
                    <div id="message" class="updated cws-gpp-message">
                        <div class="squeezer">
                            ...
                            <h4><?php _e( '<strong>GET 25% OFF! &#8211; use discount code WPMEGA25 on checkout</strong>', 'cws_gpp_' ); ?></h4>
                            <p class="submit">
